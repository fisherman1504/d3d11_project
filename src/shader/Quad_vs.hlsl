...
// Set manually for texture visualization quad code.
cbuffer VS_MP_BUFFER : register(b2) {
	float4x4 texVisModelMat;
	float4x4 texVisProjectionMat;
};

// Entry point of shader.
ps_in main(vs_in input) {
	// Coordinate system transformation.
	float4 worldCoord = mul(float4(input.position, 1.0f), texVisModelMat);
	float4 screenCoord = mul(worldCoord, texVisProjectionMat);
